...
| 項目 | 表示内容 | データソース |
|------|----------|------------|
| サムネイル | 画像（任意） | `scenarioImageUrl` |
| シナリオ名 | テキスト | `name` |
| システム名 | バッジ形式 | `scenarioSystems.name` |
| 作者名 | テキスト | `author` |
| 平均評価 | ★4.2 / 42件 | `userReviews`から集計 |
| プレイ人数 | 3-5人 | `minPlayer` - `maxPlayer` |
| プレイ時間 | 4-6時間 | `minPlaytime` - `maxPlaytime` |
| ハンドアウト | バッジ形式 | `handoutType` |
| タグ | チップ形式 | `tags`テーブル |
| 配布URL | リンクボタン | `distributeUrl` |

#### [2] アクションエリア（ログイン時のみ表示）

- **セッションを作成**: `/sessions/new?scenarioId=[id]` へ遷移
- **お気に入り★**: `userScenarioPreferences.isLike` をトグル（リアルタイム更新）
- **編集**: `/scenarios/[id]/edit` へ遷移（登録者・MODERATOR のみ表示）

#### [3] 経験登録エリア（ログイン時のみ表示）

チェックボックス4つでユーザーの経験を登録:

| チェックボックス | フィールド | 用途 |
|-----------------|-----------|------|
| ☑ プレイ済み | `isPlayed` | ネタバレ保護、経験管理 |
| ☑ 観戦済み | `isWatched` | 視聴経験の記録 |
| ☑ GMできる | `canKeeper` | GM募集時の検索 |
| ☑ シナリオ所持 | `hadScenario` | マッチング補助 |

- リアルタイム更新（onChange で即座に DB 更新）
- `userScenarioPreferences` テーブルに保存
...
| 投稿者名 | `users.nickname` |
| 評価 | ★1〜5（`rating`） |
| 公開コメント | `openComment`（常時表示） |
| スポイラーコメント | `spoilerComment`（折りたたみ、クリックで展開） |
...
|------|------|
| プレイ日 | セッション確定日時 |
| GM名 | `users.nickname`（keeperId） |
| 参加者数 | `sessionParticipants` のカウント |
| ステータス | `sessionPhase` |

##### 機能
- 「詳細を見る」ボタンで `/sessions/[id]` へ遷移
- セッション詳細ページで参加メンバー・キャラシ等を確認可能（特別な「振り返り機能」は不要）

#### [7] 関連動画セクション

- `videoLinks` テーブルから該当シナリオの動画を取得
- YouTube埋め込み or リンク形式
- 投稿者名（`createdById`）を表示
...
US-112: ユーザーとして、シナリオの関連セッションを確認できる
US-113: ユーザーとして、シナリオの関連動画を閲覧できる
US-114: 登録者・MODERATORとして、シナリオを編集できる
```
