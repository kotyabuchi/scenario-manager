...
このプロジェクトはTDD（テスト駆動開発）手法を採用しています。
...
| スキル | フェーズ | 入力 | 出力 |
|--------|---------|------|------|
| `/requirements` | 要件定義 | 機能概要 | 詳細仕様書（`.claude/requirements/`） |
| `/gen-test` | Red | 要件定義書 | 失敗するテスト |
| `/implement-tests` | Green | 失敗テスト | 最小実装 |
| `/refactor` | Refactor | 実装コード | リファクタ済みコード |

### バグ修正フロー

バグ修正は `/fix-bug` スキルを使用し、以下の流れで行う:

```
[バグ報告] → テストケース見直し → 再現テスト作成（Red）→ 修正（Green）
```
