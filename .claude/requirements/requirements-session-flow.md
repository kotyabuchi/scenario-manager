...
| 項目 | 説明 |
|------|------|
| 募集文 | 自由記述（500文字以内）。唯一の必須項目 |

#### オプション項目

| 項目 | 入力形式 | デフォルト |
|------|----------|------------|
| シナリオ | 既存シナリオから選択 or 新規登録 | 未定 |
...
| タイプ | 説明 |
|--------|------|
| プレイヤー | セッションに参加してキャラクターを操作する |
| 観戦者 | セッションを見学のみ（キャラシ不要） |
...
US-S107: GMとして、参加者のDiscord IDで事前連絡できる
...
```
US-S201: GMとして、カレンダーで日程候補を選択できる
US-S202: PLとして、候補日時に○×△で回答できる
US-S203: GMとして、集計結果を見て最適な日時を確定できる
US-S204: GMとして、日程変更（リスケ）を簡単に申請できる
US-S205: PLとして、リスケ時に代替候補を提案されて再回答できる
US-S206: 参加者として、リスケ時に前回の回答が引き継がれる
US-S207: 参加者として、日程確定の通知を受け取れる
```
...
| recruitedPlayerCount | integer | - | 募集人数（未定の場合はnull） |
