...
export const ImportScenarioForm = ({
  ...
  const initialMaxPlaytime = fieldValue(parsedData.maxPlaytime)
    ? Math.round((fieldValue(parsedData.maxPlaytime) ?? 180) / 60)
    : 180;
  ...
  const {
    ...
    defaultValues: {
      ...
      // 秒→分に変換してフォームにセット（既存フォームと同じ分単位管理）
      minPlaytime: fieldValue(parsedData.minPlaytime)
        ? Math.round((fieldValue(parsedData.minPlaytime) ?? 0) / 60)
        : undefined,
      maxPlaytime: fieldValue(parsedData.maxPlaytime)
        ? Math.round((fieldValue(parsedData.maxPlaytime) ?? 0) / 60)
        : undefined,
      scenarioSystemId: '',
  ...
  const titleReadonly = isFieldReadonly(parsedData.title);
  const authorReadonly = isFieldReadonly(parsedData.author);
  const playerReadonly = isFieldReadonly(parsedData.minPlayer);
  const playtimeReadonly = isFieldReadonly(parsedData.minPlaytime);
  ...
  return (
    <div className={styles.form_card}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={styles.form_sectionFields}>
            ...
            >
              <div className={styles.form_readonlyField}>
                <Input
                  ...
                  className={
                    titleReadonly ? styles.form_readonlyInput : undefined
                  }
                />
                {titleReadonly && (
                      ...
                      parsedData.minPlaytime || parsedData.maxPlaytime
                        ? `${
                            parsedData.minPlaytime
                              ? `${Math.round(parsedData.minPlaytime.value / 60)}分`
                              : ''
                          }${parsedData.minPlaytime && parsedData.maxPlaytime ? '〜' : ''}${
                            parsedData.maxPlaytime
                              ? `${Math.round(parsedData.maxPlaytime.value / 60)}分`
                              : ''
                          }`
                        : '未設定'
                    }
        ...
        {/* 手動入力セクション */}
        <p className={styles.form_sectionTitle_mt}>手動入力</p>
        <hr className={styles.form_divider} />
        <div className={styles.form_sectionFields}>
          {/* 画像 + フィールド群 */}
          <div className={styles.form_imageSection}>
            <div className={styles.form_imageArea}>
              {!isNil(previewUrl) ? (
            ...
            <div className={styles.form_fieldsArea}>
              {/* システム + ハンドアウト */}
